...
serveWithOptions(async (req) => {
  const { key, from, topic, author, messageId, message, rich } = await req
    .json();
  if (!key || !from || !topic || !messageId) throw new Error("Invalid request");
  if (key !== messageBridgeSecureKey) throw new Error("Invalid key");

  if (from === "krew") {
    const { data, error } = await supabase.from("topic_chat_messages")
      .select(
    ...
    await messageSyncerTelegramBot.api.sendMessage(
      ...
      `${message.author.display_name} from Krew: ${message.message}`,
      {
    ...
    await fetch(discordWebhookUrl, {
      ...
      body: JSON.stringify({
        username: `${message.author.display_name} from Krew`,
        avatar_url: avatar,
